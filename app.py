# ============================================================
# 🌞 DASHBOARD EGAUGE MULTI-PROYECTOS PARA FLY.IO
# Version: Premium completo
# ============================================================
...
def fetch_egauge_snapshot(base_url, user=EGAUGE_USER, password=EGAUGE_PASS):
    try:
...
def get_project_card(project, theme="light"):
    ...
    ts, data = fetch_egauge_snapshot(project["base_url"], project.get("user", EGAUGE_USER), project.get("password", EGAUGE_PASS))
    if not data:
    ...
    export_button = dbc.Button("📥 Exportar CSV", id={"type":"btn_export","index":project["name"]}, color="primary", className="mt-2 mb-3 btn-lg")
...
app.layout = html.Div([
    dbc.Container([
        dbc.Row([
            ...
            dbc.Col([dbc.Switch(id="theme-switch", className="mt-4"), html.Span(id="theme-label", className="ms-2 fw-bold")], width=2)
        ], align="center"),
        dcc.Store(id="theme-store", data="light", storage_type="local"),
        dcc.Store(id="store-projects", data=[{"name":"Proyecto_1","base_url":"https://egauge82252.egaug.es/632E1/","lat":18.4861,"lon":-69.9312}], storage_type="local"),
        dbc.Row([
            dbc.Col([
            ...
            dbc.Col([dbc.Tabs(id="tabs_projects", active_tab="Proyecto_1", className="mt-2"), html.Div(id="map-container", className="mt-3")], xs=12, sm=12, md=8, lg=9, xl=9)
        ], className="g-3"),
...
def add_project(n,name,url,user,password,lat,lon,projects_data):
    if not name or not url: return dash.no_update, "Debe completar nombre y URL"
...
def update_tabs_and_map(projects_data,n,theme):
    ...
    for p in projects_data:
        ts,data=fetch_egauge_snapshot(p["base_url"],p.get("user",EGAUGE_USER),p.get("password",EGAUGE_PASS))
        alarms = detect_alarms(data)
        has_error = any(a["status"]!="OK" for a in alarms)
        lats.append(p.get("lat"))
        lons.append(p.get("lon"))
        names.append(p["name"])
        colors_map.append("red" if has_error else "green")
        sizes.append(25 if has_error else 15)

    map_fig=go.Figure(go.Scattermapbox(lat=lats, lon=lons, mode='markers+text',
                                       marker=go.scattermapbox.Marker(size=sizes, color=colors_map),
                                       text=names, textposition="top right"))
    map_fig.update_layout(mapbox_style="open-street-map",
                          mapbox_zoom=5,
                          mapbox_center={"lat":np.mean(lats) if lats else 18.5, "lon":np.mean(lons) if lons else -69.9},
...
def refresh_table_graph(n,projects_data,graph_id,theme):
    ...
    for p in projects_data:
        if p["name"]==project_name:
            ...
            fig.update_layout(title=f"{project_name} - Lectura {ts.strftime('%Y-%m-%d %H:%M:%S')}",
                              xaxis_title="Canal",yaxis_title="Potencia (W)",
                              plot_bgcolor=colors["card"],paper_bgcolor=colors["card"],
                              font=dict(family="Arial",size=14,color=colors["text"]),
                              transition=dict(duration=700,easing="cubic-in-out"))
...
def export_csv(n_clicks_list,projects_data):
    ...
    if not ctx.triggered: return [dash.no_update]*len(projects_data)
    triggered_id=ctx.triggered[0]["prop_id"].split(".")[0]
    proj_name=eval(triggered_id)["index"]
    for p in projects_data:
        if p["name"]==proj_name:
            ts,data=fetch_egauge_snapshot(p["base_url"],p.get("user",EGAUGE_USER),p.get("password",EGAUGE_PASS))
            if not data: return [dash.no_update]*len(projects_data)
            df=pd.DataFrame(list(data.items()), columns=["Canal","Potencia (W)"])
            return [dcc.send_data_frame(df.to_csv,f"{proj_name}_{ts.strftime('%Y%m%d_%H%M%S')}.csv") if p["name"]==proj_name else dash.no_update for p in projects_data]
